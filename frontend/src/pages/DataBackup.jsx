...
const DataBackup = () => {
  ...
  const handleExportReports = async () => {
    ...
    try {
      ...
      // Note: Route /admin/export-reports not added yet, using toast for now
      setExporting(false);
    } catch (err) {
    ...
    <div className="p-8 min-h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50 dark:from-gray-900 dark:via-gray-800 dark:to-indigo-900">
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 rounded-3xl bg-gradient-to-r from-blue-600 to-purple-600 p-10 text-white shadow-2xl">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Data & Backup Center</h1>
          <p className="text-xl opacity-90">Comprehensive database management and backup solutions for system administrators.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
            ...
            <div className="flex items-center mb-6">
              <div className="w-12 h-12 bg-blue-500 rounded-2xl flex items-center justify-center mr-4 shadow-lg">
                <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4m-8 10V5" />
                </svg>
              </div>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Database Export</h2>
            </div>
            <p className="text-gray-600 dark:text-gray-300 mb-8 leading-relaxed">Generate complete database backup including all users, reports, and system data.</p>
            <button
          ...
          <div className="group bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl border border-white/20 dark:border-gray-700/50 rounded-3xl p-8 shadow-2xl hover:shadow-3xl hover:-translate-y-2 transition-all duration-500">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-8 leading-relaxed">Download consolidated reports and analytics data in JSON format.</p>
            <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="group bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl border border-white/20 dark:border-gray-700/50 rounded-3xl p-8 shadow-2xl hover:shadow-3xl hover:-translate-y-2 transition-all duration-500 col-span-full lg:col-span-1">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-8 leading-relaxed">View detailed system activity logs and audit trail.</p>
            <button
onClick={() => toast.success("System logs view - Backend API ready")}
          ...
          <div className="group bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl border border-white/20 dark:border-gray-700/50 rounded-3xl p-8 shadow-2xl hover:shadow-3xl hover:-translate-y-2 transition-all duration-500 col-span-full lg:col-span-1">
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Backup History</h2>
            </div>
            <p className="text-gray-600 dark:text-gray-300 mb-8 leading-relaxed">Review previous backups and restore from history.</p>
            <button
...
{new Date(backup.date).toLocaleDateString()} - {backup.size}
                      <span className={`px-2 py-1 rounded-full text-xs ${backup.status === 'Success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                        {backup.status}
                      </span>
                    </li>
                  ))}
